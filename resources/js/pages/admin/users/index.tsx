...
export default function UsersIndex({ users, filters }: Props) {
    ...
    function handleDelete() {
        ...
        router.delete(`/admin/users/${deleteTarget.id}`, { onSuccess: () => setDeleteTarget(null) });
    }
            ...
            <div className="flex flex-col gap-6 p-6">
                <div className="flex items-center justify-between">
                    <div>
                        ...
                        <p className="text-sm text-muted-foreground">Manage staff and students.</p>
                    </div>
                    <Link href="/admin/users/invite">
                        <Button><UserPlus className="mr-2 size-4" />Add User</Button>
                    </Link>
                </div>
                    ...
                    <CardContent className="pt-4">
                        <form onSubmit={handleSearch} className="flex gap-2">
                            ...
                            <div className="ml-auto flex gap-2">
                                {['', 'staff', 'student'].map((r) => (
                                    <Button key={r} size="sm" type="button"
                                        variant={filters.role === r || (!filters.role && r === '') ? 'default' : 'outline'}
                ...
                <Card>
                    <CardHeader><CardTitle className="text-base">{users.data.length} users</CardTitle></CardHeader>
                    <CardContent>
                                    ...
                                    {users.data.length === 0 ? (
                                        ...
                                        users.data.map((u) => (
                                            <tr key={u.id} className="hover">
                                                ...
                                                <td><span className={`badge badge-sm capitalize ${u.role === 'staff' ? 'badge-info' : 'badge-ghost'}`}>{u.role}</span></td>
                        ...
                        {users.last_page > 1 && (
                            <div className="mt-4 flex justify-center gap-2">
                                ...
                                <span className="flex items-center text-sm text-muted-foreground">Page {users.current_page} of {users.last_page}</span>
