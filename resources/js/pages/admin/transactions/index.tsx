...
export default function TransactionsIndex({ transactions, filters }: Props) {
    ...
    function handleReturn() {
        ...
        router.post(`/admin/transactions/${returnTarget.id}/return`, {
            return_condition_notes: notes,
            fine_amount: fine,
        }, { onSuccess: () => { setReturnTarget(null); setNotes(''); setFine(''); } });
    }
            ...
            <div className="flex flex-col gap-6 p-6">
                <div className="flex items-center justify-between">
                    <div>
                        ...
                        <p className="text-sm text-muted-foreground">Track active loans and returns.</p>
                    </div>
                    <div className="flex gap-2">
                        {['', 'active', 'overdue', 'returned'].map((s) => (
                            <Button
                                ...
                                size="sm"
                                onClick={() => router.get('/admin/transactions', { status: s }, { preserveState: true })}
                            >
                            ...
                            <table className="table table-sm w-full">
                                ...
                                <tbody>
                                    {transactions.data.length === 0 ? (
                                        <tr><td colSpan={7} className="py-8 text-center text-muted-foreground">No transactions found.</td></tr>
                                    ) : (
